...
display "F025_STATA_INPUT=`input_csv'"
display "F025_STATA_OUTPUT=`output_dir'"
display "F025_STATA_ADO_ROOT=`ado_root'"
...
did_imputation Y unit t Ei if keep == 1 [aw=w], controls(x1 x2) fe(group t) minn(0) cluster(clust) maxit(1000)
...
forvalues idx = 1/`term_count' {
    ...
    local nt = .
    if "`term'" == "tau" local nt = el(Nt, 1, 1)
    file write estimates "`term'," %21.17g (`estimate') "," %21.17g (`std_error') "," %21.17g (`conf_low') "," %21.17g (`conf_high') "," %21.17g (e(N)) "," %21.17g (e(Nc)) "," %21.17g (`nt') _n
}
...
file write mask "row_id,sample,keep,missing_required" _n
forvalues r = 1/`=_N' {
    local missing_required = missing(Y[`r']) | missing(x1[`r']) | missing(x2[`r']) | missing(group[`r']) | missing(clust[`r']) | missing(w[`r'])
    file write mask "`=row_id[`r']'," %21.17g (sample[`r']) "," %21.17g (keep[`r']) "," %21.17g (`missing_required') _n
}
file close mask

quietly count if keep == 0
local subset_excluded = r(N)
quietly count if keep == 1 & (missing(Y) | missing(x1) | missing(x2) | missing(group) | missing(clust) | missing(w))
local missing_excluded = r(N)

file open diag using "`output_dir'/diagnostics.json", write replace
file write diag "{" _n
file write diag `"  "status": "success","' _n
file write diag `"  "command": "did_imputation Y unit t Ei if keep == 1 [aw=w], controls(x1 x2) fe(group t) minn(0) cluster(clust) maxit(1000)","' _n
...
file write diag `"  "subset_excluded": "' %21.17g (`subset_excluded') "," _n
file write diag `"  "missing_excluded": "' %21.17g (`missing_excluded') _n
file write diag "}" _n
file close diag

display "F025_STATA_EXPORT_OK=1"
log close
exit, clear
