...
display "F011_STATA_INPUT=`input_csv'"
display "F011_STATA_OUTPUT=`output_dir'"
display "F011_STATA_ADO_ROOT=`ado_root'"
...
egen byte ig_tag = tag(ig)
quietly count if ig_tag
local unique_ig = r(N)
drop ig_tag
egen byte county_tag = tag(county)
quietly count if county_tag
local unique_counties = r(N)
drop county_tag
egen byte g_tag = tag(g)
quietly count if g_tag
local unique_groups = r(N)
drop g_tag
quietly count
...
did_imputation Y ig t Eig, fe(ig county#t g#t) minn(0) cluster(county) saveweights
...
file write diag `"  "command": "did_imputation Y ig t Eig, fe(ig county#t g#t) minn(0) cluster(county) saveweights","' _n
file write diag `"  "unit_var": "ig","' _n
file write diag `"  "fe": ["ig", "county#t", "g#t"],"' _n
file write diag `"  "cluster": "county","' _n
file write diag `"  "total_rows": "' %21.17g (`total_rows') "," _n
file write diag `"  "unique_composite_units": "' %21.17g (`unique_ig') "," _n
file write diag `"  "unique_counties": "' %21.17g (`unique_counties') "," _n
file write diag `"  "unique_groups": "' %21.17g (`unique_groups') "," _n
...
display "F011_STATA_EXPORT_OK=1"
log close
exit, clear
