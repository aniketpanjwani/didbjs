...
display "F017_STATA_INPUT=`input_csv'"
display "F017_STATA_OUTPUT=`output_dir'"
display "F017_STATA_ADO_ROOT=`ado_root'"
...
preserve
capture noisily did_imputation Y unit t Ei, minn(0) cluster(unit) leaveout
local failure_rc = _rc
if (`failure_rc' != 498) {
    display as error "Expected default leaveout to fail with return code 498, got `failure_rc'"
    exit 498
}
restore

file open failure using "`output_dir'/default-error.json", write replace
file write failure "{" _n
file write failure `"  "status": "error","' _n
file write failure `"  "command": "did_imputation Y unit t Ei, minn(0) cluster(unit) leaveout","' _n
file write failure `"  "return_code": "' %21.17g (`failure_rc') "," _n
file write failure `"  "error_message": "Cannot compute leave-out standard errors because of 10 observations for coefficient \"tau\"","' _n
file write failure `"  "guidance_1": "This most likely happened because there are cohorts with only one unit or cluster (and the default value for avgeffectsby  is used).","' _n
file write failure `"  "guidance_2": "Consider using the avgeffectsby option with broader observation groups. Do not address this problem by using non-leave-out standard errors, as they may be downward biased for the same reason.""' _n
file write failure "}" _n
file close failure

did_imputation Y unit t Ei, minn(0) cluster(unit) avgeffectsby(D) leaveout
...
file write diag `"  "command": "did_imputation Y unit t Ei, minn(0) cluster(unit) avgeffectsby(D) leaveout","' _n
file write diag `"  "avgeffectsby": ["D"],"' _n
file write diag `"  "leaveout": true,"' _n
file write diag `"  "default_error_return_code": "' %21.17g (`failure_rc') "," _n
...
display "F017_STATA_EXPORT_OK=1"
log close
exit, clear
