#!/usr/bin/env python3
"""Generate Python reference artifacts for F027 irregular/unbalanced panels."""
...
def run_reference(panel: pd.DataFrame):
...
def main() -> int:
    ...
    parser.add_argument("duplicate_csv", type=pathlib.Path)
    ...
    with contextlib.redirect_stdout(log_buffer), contextlib.redirect_stderr(log_buffer):
        print(f"F027_PYTHON_INPUT={args.input_csv}")
        print(f"F027_PYTHON_DUPLICATE_INPUT={args.duplicate_csv}")
        print(f"F027_PYTHON_OUTPUT={args.output_dir}")
        print(f"PYTHON_VERSION={sys.version}")
        panel = pd.read_csv(args.input_csv)
        output = run_reference(panel)
        duplicate_panel = pd.read_csv(args.duplicate_csv)
        duplicate_rows = int(duplicate_panel.duplicated(["unit", "t"], keep=False).sum())
        try:
            duplicate_output = run_reference(duplicate_panel)
            duplicate_probe = {
                "status": "reference_success_with_duplicates",
                "duplicate_unit_time_rows": duplicate_rows,
                "estimate": float(duplicate_output.estimates["tau_ate"]),
                "n_obs": int(duplicate_output.n_obs),
            }
    ...
    diagnostics = {
        "status": "success",
        "command": 'did_imputation(panel, y="Y", i="unit", t="t", Ei="Ei", fe=["unit", "t"], minn=0, cluster="unit")',
        "python_version": sys.version,
        "platform": platform.platform(),
        "input_sha256": sha256(args.input_csv),
        "duplicate_input_sha256": sha256(args.duplicate_csv),
    ...
    (args.output_dir / "duplicate-probe.json").write_text(json.dumps(duplicate_probe, indent=2) + "\n")
    return 0
