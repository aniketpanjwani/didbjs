...
display "F009_STATA_INPUT=`input_csv'"
display "F009_STATA_OUTPUT=`output_dir'"
display "F009_STATA_ADO_ROOT=`ado_root'"
...
did_imputation Y unit t Ei [aw=w], controls(x1 x2) minn(0) cluster(unit) maxit(1000) saveweights
...
forvalues idx = 1/`term_count' {
    ...
    if "`term'" == "tau" {
        file write estimates "`term'," %21.17g (`estimate') "," %21.17g (`std_error') "," %21.17g (`conf_low') "," %21.17g (`conf_high') "," %21.17g (e(N)) "," %21.17g (e(Nc)) "," %21.17g (el(Nt, 1, 1)) _n
    }
    else {
        file write estimates "`term'," %21.17g (`estimate') "," %21.17g (`std_error') "," %21.17g (`conf_low') "," %21.17g (`conf_high') "," %21.17g (e(N)) "," %21.17g (e(Nc)) "," _n
    }
}
...
local x1_estimate = el(b, 1, 2)
local x2_estimate = el(b, 1, 3)
local tau_se = sqrt(el(V, 1, 1))
local x1_se = sqrt(el(V, 2, 2))
local x2_se = sqrt(el(V, 3, 3))
local n_obs = e(N)
local n_control = e(Nc)
local n_treated = el(Nt, 1, 1)

preserve
capture noisily did_imputation Y unit t Ei [aw=w], controls(x_bad) minn(0) cluster(unit) maxit(1000)
local collinear_rc = _rc
restore

file open coll using "`output_dir'/collinearity.json", write replace
file write coll "{" _n
if (`collinear_rc' == 0) {
    file write coll `"  "status": "unexpected_success","' _n
}
else {
    file write coll `"  "status": "error","' _n
}
file write coll `"  "return_code": "' %21.17g (`collinear_rc') _n
file write coll "}" _n
file close coll
...
file write diag `"  "command": "did_imputation Y unit t Ei [aw=w], controls(x1 x2) minn(0) cluster(unit) maxit(1000) saveweights","' _n
file write diag `"  "controls": ["x1", "x2"],"' _n
file write diag `"  "terms": ["tau", "x1", "x2"],"' _n
file write diag `"  "estimates": {"' _n
file write diag `"    "tau": "' %21.17f (`tau_estimate') "," _n
file write diag `"    "x1": "' %21.17f (`x1_estimate') "," _n
file write diag `"    "x2": "' %21.17f (`x2_estimate') _n
file write diag `"  }, "' _n
file write diag `"  "std_errors": {"' _n
file write diag `"    "tau": "' %21.17f (`tau_se') "," _n
file write diag `"    "x1": "' %21.17f (`x1_se') "," _n
file write diag `"    "x2": "' %21.17f (`x2_se') _n
file write diag `"  }, "' _n
...
file write diag `"  "collinearity_return_code": "' %21.17g (`collinear_rc') _n
file write diag "}" _n
file close diag

display "F009_STATA_EXPORT_OK=1"
log close
exit, clear
