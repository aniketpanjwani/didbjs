...
display "F013_STATA_INPUT=`input_csv'"
display "F013_STATA_OUTPUT=`output_dir'"
display "F013_STATA_ADO_ROOT=`ado_root'"
...
preserve
capture noisily did_imputation Y unit t Ei, horizons(0/2) minn(0) cluster(unit)
local default_rc = _rc
local default_cannot_count = .
capture confirm variable cannot_impute
if (_rc == 0) {
    quietly count if cannot_impute == 1
    local default_cannot_count = r(N)
    file open cannot_default using "`output_dir'/cannot-impute-default.csv", write replace
    file write cannot_default "row_id,cannot_impute" _n
    forvalues r = 1/`=_N' {
        file write cannot_default "`=row_id[`r']'," %21.17g (cannot_impute[`r']) _n
    }
    file close cannot_default
}
restore

file open default_error using "`output_dir'/default-error.json", write replace
file write default_error "{" _n
if (`default_rc' == 0) {
    file write default_error `"  "status": "unexpected_success","' _n
}
else {
    file write default_error `"  "status": "error","' _n
}
file write default_error `"  "return_code": "' %21.17g (`default_rc') "," _n
file write default_error `"  "cannot_impute_count": "' %21.17g (`default_cannot_count') _n
file write default_error "}" _n
file close default_error

did_imputation Y unit t Ei, horizons(0/2) autosample minn(0) cluster(unit) saveweights
...
local autosample_drop "`e(autosample_drop)'"
local autosample_trim "`e(autosample_trim)'"
...
file write mask "row_id,sample,cannot_impute" _n
forvalues r = 1/`=_N' {
    file write mask "`=row_id[`r']'," %21.17g (sample[`r']) "," %21.17g (cannot_impute[`r']) _n
}
file close mask
...
file write diag `"  "command": "did_imputation Y unit t Ei, horizons(0/2) autosample minn(0) cluster(unit) saveweights","' _n
file write diag `"  "terms": ["tau0", "tau1", "tau2"],"' _n
file write diag `"  "autosample": true,"' _n
file write diag `"  "autosample_drop": "`autosample_drop'","' _n
file write diag `"  "autosample_trim": "`autosample_trim'","' _n
file write diag `"  "droplist": "`droplist'","' _n
file write diag `"  "default_return_code": "' %21.17g (`default_rc') "," _n
file write diag `"  "cannot_impute_count": "' %21.17g (`default_cannot_count') "," _n
...
display "F013_STATA_EXPORT_OK=1"
log close
exit, clear
