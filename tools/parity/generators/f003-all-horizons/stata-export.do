...
display "F003_STATA_INPUT=`input_csv'"
display "F003_STATA_OUTPUT=`output_dir'"
display "F003_STATA_ADO_ROOT=`ado_root'"
...
did_imputation Y unit t Ei [aw=w], allhorizons minn(0) cluster(unit)
...
forvalues j = 1/`=colsof(b)' {
    ...
    if abs(`estimate' - `target') > 1e-10 {
        display as error "F003 allhorizons ATT assertion failed for `term': " %21.17g `estimate'
        exit 459
    }
}
...
file write diag `"  "command": "did_imputation Y unit t Ei [aw=w], allhorizons minn(0) cluster(unit)","' _n
file write diag `"  "stata_version": "`=c(stata_version)'","' _n
file write diag `"  "discovered_terms": ["tau0", "tau1", "tau2"],"' _n
...
display "F003_STATA_EXPORT_OK=1"
log close
exit, clear
