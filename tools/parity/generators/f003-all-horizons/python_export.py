#!/usr/bin/env python3
"""Generate Python reference artifacts for F003 all-horizon discovery."""
...
def main() -> int:
    ...
    with contextlib.redirect_stdout(log_buffer), contextlib.redirect_stderr(log_buffer):
        print(f"F003_PYTHON_INPUT={args.input_csv}")
        print(f"F003_PYTHON_OUTPUT={args.output_dir}")
        ...
        if list(output.estimates.keys()) != list(expected.keys()):
            raise AssertionError(f"F003 allhorizons terms mismatch: {list(output.estimates.keys())}")
        for term, target in expected.items():
            estimate = float(output.estimates[term])
            if abs(estimate - target) > 1e-5:
                raise AssertionError(f"F003 {term} assertion failed: {estimate}")
        print("F003_PYTHON_EXPORT_OK=1")
    ...
    diagnostics = {
        "status": status,
        "command": 'did_imputation(panel, y="Y", i="unit", t="t", Ei="Ei", fe=["unit", "t"], aw="w", allhorizons=True, minn=0)',
    ...
    if output is not None:
        diagnostics["discovered_terms"] = list(output.estimates.keys())
        ...
        if not diagnostics["tol001_pass"]:
            diagnostics["root_cause_probe"] = (
                "Same fixed-effect recovery drift as D017/F001/F002: pinned "
                "Python recover_fixed_effects_iterative() leaves discovered "
                "horizon estimates outside TOL001 while preserving public "
                "allhorizons object shape."
            )
