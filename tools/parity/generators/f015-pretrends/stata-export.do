...
display "F015_STATA_INPUT=`input_csv'"
display "F015_STATA_OUTPUT=`output_dir'"
display "F015_STATA_ADO_ROOT=`ado_root'"
...
did_imputation Y unit t Ei, horizons(0/1) pretrends(2) minn(0) cluster(unit) saveweights
...
forvalues idx = 1/`term_count' {
    ...
    local n_treated = .
    if `idx' <= colsof(Nt) local n_treated = el(Nt, 1, `idx')
    file write estimates "`term'," %21.17g (`estimate') "," %21.17g (`std_error') "," %21.17g (`conf_low') "," %21.17g (`conf_high') "," %21.17g (e(N)) "," %21.17g (e(Nc)) "," %21.17g (`n_treated') _n
}
...
file write diag `"  "command": "did_imputation Y unit t Ei, horizons(0/1) pretrends(2) minn(0) cluster(unit) saveweights","' _n
file write diag `"  "terms": ["tau0", "tau1", "pre1", "pre2"],"' _n
file write diag `"  "pretrends": 2,"' _n
file write diag `"  "pre_F": "' %21.17g (e(pre_F)) "," _n
file write diag `"  "pre_p": "' %21.17g (e(pre_p)) "," _n
file write diag `"  "pre_df": "' %21.17g (e(pre_df)) "," _n
file write diag `"  "n_obs": "' %21.17g (e(N)) "," _n
file write diag `"  "n_control": "' %21.17g (e(Nc)) _n
file write diag "}" _n
file close diag

display "F015_STATA_EXPORT_OK=1"
log close
exit, clear
