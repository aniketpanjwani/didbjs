...
generator_path <- if (length(file_arg)) {
  ...
  "tools/parity/generators/f015-pretrends/kyle-export.R"
}
...
cat("F015_KYLE_INPUT=", input_csv, "\n", sep = "")
cat("F015_KYLE_OUTPUT=", output_dir, "\n", sep = "")
...
diagnostics <- list(
  ...
  command = 'did_imputation(data = panel, yname = "Y", gname = "Ei", tname = "t", idname = "unit", horizon = 0:1, pretrends = c(-2, -1), cluster_var = "unit")',
...
if (inherits(result, "error")) {
  ...
  diagnostics$terms <- as.character(result$term)
  cat("F015_KYLE_EXPORT_OK=1\n")
}
