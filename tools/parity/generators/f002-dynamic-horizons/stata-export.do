...
display "F002_STATA_INPUT=`input_csv'"
display "F002_STATA_OUTPUT=`output_dir'"
display "F002_STATA_ADO_ROOT=`ado_root'"
...
did_imputation Y unit t Ei [aw=w], horizons(0/2) minn(0) cluster(unit)
...
forvalues j = 1/`=colsof(b)' {
    ...
    if abs(`estimate' - `target') > 1e-10 {
        display as error "F002 dynamic ATT assertion failed for `term': " %21.17g `estimate'
        exit 459
    }
}
...
file write diag `"  "command": "did_imputation Y unit t Ei [aw=w], horizons(0/2) minn(0) cluster(unit)","' _n
...
display "F002_STATA_EXPORT_OK=1"
log close
exit, clear
